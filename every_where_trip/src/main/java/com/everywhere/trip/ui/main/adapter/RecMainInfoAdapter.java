...
public class RecMainInfoAdapter extends RecyclerView.Adapter {
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        ...
        if (viewType == IMAGE_TYPE) {
            ...
            final List<MainDataInfo.ResultEntity.ReviewsEntity> list = resultEntity.getReviews();
            ...
            if (list.get(position - 2).getImages() != null && list.get(position - 2).getImages().size() != 0) {
                ...
                adapter.setOnImageClickListener(new RecImageAdapter.OnImageClickListener() {
                    @Override
                    public void onClick(String imgUrl,int position) {
                        if (onImageClickListener != null){
                            onImageClickListener.onClick(list.get(position - 2).getImages(),position);
                        }
                    }
                });
            }
    ...
    public interface OnImageClickListener{
        void onClick(List<String> imgUrl,int position);
    }
